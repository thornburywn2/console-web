...
function Toggle({ checked, onChange, label, description, accentColor = 'primary' }) {
  const colors = {
...
function InputField({ label, value, onChange, placeholder, description, type = 'text', required, error }) {
  return (
...
function SummaryItem({ icon, label, value, status }) {
  const statusColors = {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (createGitHubRepo && githubAvailable) {
        ...
        try {
          // First, ensure the project exists in database
